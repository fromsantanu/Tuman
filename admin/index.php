...
require_once dirname(__DIR__) . '/services/profiles.php';
$current=tuman_require_role('ADMIN'); $profile=tuman_profile(tuman_database(),$current['id'],'ADMIN'); $name=trim(($profile['first_name']??'').' '.($profile['last_name']??'')) ?: ($profile['username']??'Administrator');
...
<p><a class="button" href="/Tuman/admin/users.php">View users</a> <a class="button" href="/Tuman/admin/user-create.php">Create user</a> <a class="button" href="/Tuman/admin/maintenance.php">Backups and retention</a></p>
