...
$error = null;
if ($_SERVER['REQUEST_METHOD'] === 'POST') { if (!tuman_csrf_is_valid($_POST['csrf_token'] ?? null)) { $error = 'Your request could not be verified. Please try again.'; } else { try { tuman_cancel_invoice(tuman_database(), $current['id'], (int) $id); header('Location: /Tuman/teacher/invoice-view.php?id=' . $id); exit; } catch (InvalidArgumentException $exception) { http_response_code(404); $error = 'Invoice unavailable.'; } catch (Throwable $exception) { error_log('Tuman invoice cancellation failed: ' . $exception->getMessage()); $error = 'Unable to cancel the invoice right now.'; } } }
tuman_teacher_page_start('Cancel draft invoice');
?>
<h1>Cancel draft invoice</h1><p>Cancel <?= htmlspecialchars($invoice['invoice_number'], ENT_QUOTES, 'UTF-8') ?>? Its header and item history will be retained, but it can no longer be issued.</p><?php if ($error): ?><p role="alert"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></p><?php endif; ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(tuman_csrf_token(), ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="id" value="<?= (int) $id ?>"><button type="submit">Cancel draft</button> <a href="/Tuman/teacher/invoice-view.php?id=<?= (int) $id ?>">Keep draft</a></form>
