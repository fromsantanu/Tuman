...
$hourlyRules = array_filter(
    tuman_teacher_billing_rules($database, $current['id'], (int) $record['teacher_student_id']),
    static fn (array $rule): bool => $rule['status'] === 'ACTIVE' && $rule['billing_mode'] === 'HOURLY'
);
$error = null;
$values = ['session_date' => $record['session_date'], 'status' => $record['status'], 'start_time' => $record['start_time'] === null ? '' : substr($record['start_time'], 0, 5), 'end_time' => $record['end_time'] === null ? '' : substr($record['end_time'], 0, 5), 'billing_rule_id' => $record['billing_rule_id'] === null ? '' : (string) $record['billing_rule_id'], 'remarks' => $record['remarks'] ?? ''];
