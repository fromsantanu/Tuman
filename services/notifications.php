...
{
    $s=$db->prepare("INSERT INTO tmn_email_log (related_entity_type,related_entity_id,recipient_email,subject,delivery_status,attempted_at,provider_response) VALUES (:type,:id,:recipient,:subject,'PENDING',NULL,NULL)");$s->execute(['type'=>$entity,'id'=>$entityId,'recipient'=>$recipient,'subject'=>$subject]);$logId=(int)$db->lastInsertId();tuman_log_activity($db,$teacherId,'EMAIL_QUEUED',$entity,$entityId,$meta);return $logId;
}

function tuman_deliver_email(PDO $db,int $logId,string $recipient,string $subject,string $body,?string $attachmentPath=null): string
{
    ...
    try { foreach(['MAIL_HOST','MAIL_PORT','MAIL_USERNAME','MAIL_PASSWORD','MAIL_FROM_ADDRESS'] as $key){if(($environment[$key]??'')===''){throw new RuntimeException('SMTP configuration is incomplete.');}}
        $mail=new PHPMailer(true);$mail->isSMTP();$mail->Host=$environment['MAIL_HOST'];$mail->Port=(int)$environment['MAIL_PORT'];$mail->SMTPAuth=true;$mail->Username=$environment['MAIL_USERNAME'];$mail->Password=$environment['MAIL_PASSWORD'];$encryption=strtolower($environment['MAIL_ENCRYPTION']??'tls');if($encryption==='ssl'){$mail->SMTPSecure=PHPMailer::ENCRYPTION_SMTPS;}elseif($encryption==='tls'){$mail->SMTPSecure=PHPMailer::ENCRYPTION_STARTTLS;}else{$mail->SMTPAutoTLS=false;}$mail->CharSet='UTF-8';$mail->setFrom($environment['MAIL_FROM_ADDRESS'],$environment['MAIL_FROM_NAME']??'Tuman');$mail->addAddress($recipient);$mail->Subject=$subject;$mail->isHTML(true);$mail->Body=nl2br(htmlspecialchars($body,ENT_QUOTES,'UTF-8'));$mail->AltBody=$body;if($attachmentPath!==null){$file=dirname(__DIR__).$attachmentPath;if(is_file($file))$mail->addAttachment($file);}$mail->send();tuman_update_email_delivery($db,$logId,'SENT','Delivered through SMTP.');return 'SENT';
