...
{
    $statement=$database->prepare("SELECT i.currency_code,i.total_amount,COALESCE((SELECT SUM(p.amount) FROM tmn_payments p WHERE p.invoice_id=i.id AND p.status='VALID'),0) paid,COALESCE((SELECT SUM(ca.amount) FROM tmn_credit_applications ca WHERE ca.invoice_id=i.id),0) credits,COALESCE((SELECT SUM(r.amount) FROM tmn_refunds r WHERE r.invoice_id=i.id AND r.refund_type='PAYMENT'),0) refunds FROM tmn_invoices i INNER JOIN tmn_teacher_students ts ON ts.id=i.teacher_student_id WHERE ts.student_user_id=:student_id AND i.status IN ('ISSUED','PARTIALLY_PAID')");$statement->execute(['student_id'=>$studentId]);$totals=[];foreach($statement->fetchAll() as $row){$currency=$row['currency_code'];$due=(tuman_invoice_minor_from_decimal((string)$row['total_amount'])??0)-(tuman_invoice_minor_from_decimal((string)$row['paid'])??0)-(tuman_invoice_minor_from_decimal((string)$row['credits'])??0)+(tuman_invoice_minor_from_decimal((string)$row['refunds'])??0);$totals[$currency]=($totals[$currency]??0)+max(0,$due);}return $totals;
}
